...
class Config(QConfig):
    ...
    llm_service = OptionsConfigItem(
        "LLM",
        "LLMService",
        LLMServiceEnum.GEMINI,  # 默认使用 Gemini
    ...
    gemini_model = ConfigItem("LLM", "Gemini_Model", "gemini-2.0-flash-exp")
    ...
    # ------------------- 素材搜索API配置 -------------------
    pexels_api_key = ConfigItem("MaterialSearch", "PexelsAPIKey", "")
    unsplash_api_key = ConfigItem("MaterialSearch", "UnsplashAPIKey", "")
    pixabay_api_key = ConfigItem("MaterialSearch", "PixabayAPIKey", "")

    # ------------------- Kling AI配置 -------------------
    kling_api_base = ConfigItem("KlingAI", "APIBase", "https://api.klingai.com")
    kling_access_key = ConfigItem("KlingAI", "AccessKey", "")
    kling_secret_key = ConfigItem("KlingAI", "SecretKey", "")
    # 端点可配置，便于根据官方文档调整
    kling_upload_endpoint = ConfigItem("KlingAI", "UploadEndpoint", "/v1/upload")
    kling_create_endpoint = ConfigItem("KlingAI", "CreateEndpoint", "/v1/video/create")
    kling_task_endpoint = ConfigItem("KlingAI", "TaskEndpoint", "/v1/video/task/{task_id}")

    # ------------------- RAG问答系统配置 -------------------
    rag_embedding_type = OptionsConfigItem(
        "RAG",
        "EmbeddingType",
        "gemini",  # 默认使用 Gemini
        OptionsValidator(["openai", "gemini", "local"])
    )
    rag_embedding_model = ConfigItem(
        "RAG",
        "EmbeddingModel",
        "gemini-embedding-001"  # 默认使用 Gemini embedding
    )
    rag_top_k = RangeConfigItem(
        "RAG",
        "TopK",
        5,
        RangeValidator(1, 20)
    )
    rag_vector_db_path = ConfigItem(
        "RAG",
        "VectorDBPath",
        ""
    )
    ...
    # ------------------- 邮件配置 -------------------
    email_smtp_server = ConfigItem("Email", "SMTP_Server", "smtp-mail.outlook.com")
    email_smtp_port = RangeConfigItem(
        "Email",
        "SMTP_Port",
        587,
        RangeValidator(1, 65535)
    )
    email_address = ConfigItem("Email", "Email_Address", "ritawang97@outlook.com")
    email_password = ConfigItem("Email", "Email_Password", "")
    email_use_tls = ConfigItem("Email", "Use_TLS", True, BoolValidator())
    email_recipient = ConfigItem("Email", "Recipient_Address", "ritawang97@outlook.com")
    
    # Email bot configuration (for receiving and auto-replying)
    email_bot_address = ConfigItem("Email", "Bot_Email_Address", "ruiiwang026@gmail.com")
    email_bot_password = ConfigItem("Email", "Bot_Email_Password", "Ritawang2003@")
    email_imap_server = ConfigItem("Email", "IMAP_Server", "imap.gmail.com")
    email_imap_port = RangeConfigItem(
        "Email",
        "IMAP_Port",
        993,
        RangeValidator(1, 65535)
    )
    email_bot_smtp_server = ConfigItem("Email", "Bot_SMTP_Server", "")
    email_bot_smtp_port = RangeConfigItem(
        "Email",
        "Bot_SMTP_Port",
        587,
        RangeValidator(1, 65535)
    )
    email_auto_reply_enabled = ConfigItem("Email", "Auto_Reply_Enabled", False, BoolValidator())
    email_check_interval = RangeConfigItem(
        "Email",
        "Check_Interval",
        60,
        RangeValidator(10, 600)
    )
    # OAuth 2.0 configuration
    email_use_oauth = ConfigItem("Email", "Use_OAuth", False, BoolValidator())
    email_oauth_client_id = ConfigItem("Email", "OAuth_Client_ID", "")
    email_oauth_client_secret = ConfigItem("Email", "OAuth_Client_Secret", "")
...
cfg.themeColor.value = QColor("#ff4a90e2")  # 更改为蓝色主题
