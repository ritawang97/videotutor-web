...
class DownloadDialog(MessageBoxBase):
    ...
    def setup_ui(self):
        self.titleLabel = BodyLabel(self.tr("Download Model"), self)
        ...
        # 下载按钮
        self.download_button = PushButton(self.tr("Download"), self)
    ...
    def start_download(self):
        ...
        if os.path.exists(save_path):
            InfoBar.warning(
                title=self.tr("Tip"),
                content=self.tr("Model file already exists, no need to download again"),
                parent=self.window(),
                duration=3000,
            )
            return
    ...
    def download_finished(self):
        InfoBar.success(
            ...
            content=self.tr("Model download completed!"),
            parent=self.window(),
            duration=3000,
        )
        self.download_button.setEnabled(True)
        self.progress_label.setText(self.tr("Download completed"))
...
class WhisperCppDownloadDialog(MessageBoxBase):
    ...
    def _setup_program_section(self, layout):
        """设置程序下载部分UI"""
        # 标题
        whisper_cpp_title = SubtitleLabel(self.tr("WhisperCpp Program"), self)
        ...
        if has_program:
            ...
            program_status = BodyLabel(self.tr(f"Installed version: {versions_text}"), self)
            program_status.setStyleSheet("color: #4a90e2")
            layout.addWidget(program_status)
        else:
            desc_label = BodyLabel(self.tr("WhisperCpp program not downloaded"), self)
    ...
    def _add_model_row(self, row, model):
        ...
        status_item = QTableWidgetItem(
            self.tr("Downloaded") if os.path.exists(model_bin_path) else self.tr("Not Downloaded")
        )
        ...
        download_btn = HyperlinkButton(
            "",
            self.tr("Re-download") if os.path.exists(model_bin_path) else self.tr("Download"),
            parent=self,
        )
...
class WhisperCppSettingWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.setting_group = SettingCardGroup(
            self.tr("Whisper CPP Settings (Unstable 🤔)"), self
        )
        ...
        self.manage_model_card = HyperlinkCard(
            ...
            self.tr("Download or Update Whisper CPP Model"),
