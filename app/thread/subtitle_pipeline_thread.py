...
class SubtitlePipelineThread(QThread):
    ...
    def run(self):
        try:
            ...
            logger.info("开始转录")
            self.progress.emit(0, self.tr("Start Transcription"))
            ...
            self.progress.emit(40, self.tr("Start Optimizing Subtitles"))
            ...
            self.progress.emit(80, self.tr("Start Synthesizing Video"))
            ...
            logger.info("处理完成")
            self.progress.emit(100, self.tr("Processing Completed"))
