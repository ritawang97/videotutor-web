...
from app.core.entities import SubtitleConfig, SubtitleTask, TranslatorServiceEnum, SplitTypeEnum
...
class SubtitleThread(QThread):
    ...
    def run(self):
        try:
            ...
            assert subtitle_path is not None, self.tr("Subtitle file path is empty")
            ...
            if (
                ...
                self.progress.emit(2, self.tr("Start verifying API configuration..."))
            ...
            if asr_data.is_word_timestamp():
                self.progress.emit(5, self.tr("Segmenting Subtitles..."))
                logger.info("正在字幕断句...")
                if not subtitle_config.llm_model:
                    raise Exception(self.tr("Subtitle segmentation requires LLM model configuration"))
                # Convert SplitTypeEnum to string ("semantic" or "sentence")
                if subtitle_config.split_type:
                    if subtitle_config.split_type == SplitTypeEnum.SENTENCE:
                        split_type_str = "sentence"
                    else:
                        split_type_str = "semantic"
                else:
                    split_type_str = "semantic"
            ...
            if subtitle_config.need_optimize:
                self.progress.emit(0, self.tr("Optimizing Subtitles..."))
                logger.info("正在优化字幕...")
                self.finished_subtitle_length = 0  # 重置计数器
                if not subtitle_config.llm_model:
                    raise Exception(self.tr("Subtitle optimization requires LLM model configuration"))
            ...
            if subtitle_config.need_translate:
                target_lang = str(subtitle_config.target_language) if subtitle_config.target_language else "Chinese"
                self.progress.emit(0, self.tr(f"Translating Subtitles to {target_lang}..."))
                logger.info(f"正在翻译字幕到 {target_lang}...")
                self.finished_subtitle_length = 0  # 重置计数器
                ...
                if subtitle_config.translator_service:
                    ...
                    if (
                        ...
                        if not subtitle_config.llm_model:
                            raise Exception(self.tr("Using OpenAI translation requires LLM model configuration"))
                    ...
                    raise Exception(self.tr("Translation service not configured"))
            ...
            self.progress.emit(100, self.tr("Optimization Completed"))
            logger.info("优化完成")
            self.finished.emit(self.task.video_path, self.task.output_path)
        except Exception as e:
            logger.exception(f"优化失败: {str(e)}")
            self.error.emit(str(e))
            self.progress.emit(100, self.tr("Optimization Failed"))
    ...
    def callback(self, result: Dict):
        ...
        self.progress.emit(progress, self.tr("{0}% Processing Subtitles").format(progress))
    ...
    def stop(self):
        ...
        try:
            ...
            # 发送进度信号
            self.progress.emit(100, self.tr("Terminated"))

        except Exception as e:
            logger.error(f"停止线程时出错：{str(e)}")
            self.progress.emit(100, self.tr("Error occurred during termination"))
