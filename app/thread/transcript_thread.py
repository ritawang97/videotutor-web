...
class TranscriptThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            #     self.progress.emit(100, self.tr("Subtitles Already Exist"))
            #     self.finished.emit(self.task)
            #     return

            # 检查视频文件是否存在
            if not self.task.file_path:
                raise ValueError(self.tr("File Path is Empty"))
            video_path = Path(self.task.file_path)
            if not video_path.exists():
                logger.error(f"视频文件不存在：{video_path}")
                raise ValueError(self.tr("Video File Does Not Exist"))
            ...
            if self.task.transcribe_config.transcribe_model in [
                ...
                if not self.service_manager.check_service_available(
                    ...
                    raise Exception(
                        self.tr("Public ASR service has reached daily usage limit, recommend using local transcription")
                    )
            ...
            if self.task.need_next_task and self.task.file_path:
                ...
                if downloaded_subtitles:
                    ...
                    self.progress.emit(100, self.tr("Subtitles Downloaded"))
                    self.finished.emit(self.task)
                    return

            self.progress.emit(5, self.tr("Converting Audio"))
            logger.info("开始转换音频")
            ...
            if not is_success:
                logger.error("音频转换失败")
                raise RuntimeError(self.tr("Audio conversion failed"))

            self.progress.emit(20, self.tr("Transcribing Speech"))
            logger.info("开始语音转录")

            # 进行转录，并回调进度。 （传入 transcribe_config）
            if not self.task.transcribe_config:
                raise ValueError(self.tr("Transcription Config is Empty"))
            asr_data = transcribe(
                temp_file.name,
            ...
            if not self.task.output_path:
                raise ValueError(self.tr("Output Path is Empty"))
            ...
            self.progress.emit(100, self.tr("Transcription Completed"))
            self.finished.emit(self.task)
        except Exception as e:
            logger.exception("转录过程中发生错误: %s", str(e))
            self.error.emit(str(e))
            self.progress.emit(100, self.tr("Transcription Failed"))
        finally:
            # 清理临时文件
