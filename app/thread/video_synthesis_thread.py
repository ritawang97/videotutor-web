    ...
    def run(self):
        try:
            ...
            if not need_video:
                ...
                self.progress.emit(100, self.tr("Synthesis Completed"))
                self.finished.emit(self.task)
                return

            logger.info(f"开始合成视频: {video_file}")
            self.progress.emit(5, self.tr("Synthesizing"))

            if not video_file:
                raise ValueError(self.tr("Video Path is Empty"))
            if not subtitle_file:
                raise ValueError(self.tr("Subtitle Path is Empty"))
            if not output_path:
            ...
            self.progress.emit(100, self.tr("Video Synthesis Failed"))
