        ...
        if role == Qt.DisplayRole:  # type: ignore
            if orientation == Qt.Horizontal:  # type: ignore
                return [
                    self.tr("Start Time"),
                    self.tr("End Time"),
                    self.tr("Subtitle Content"),
                    (
                        self.tr("Translated Subtitle")
                        if cfg.need_translate.value
                        else self.tr("Optimized Subtitle")
                    ),
                ][section]
...
class SubtitleInterface(QWidget):
    ...
    def _setup_top_layout(self):
        ...
        # 添加保存按钮(带下拉菜单)
        save_button = TransparentDropDownPushButton(self.tr("Save"), self, FIF.SAVE)
        ...
        self.layout_button = TransparentDropDownPushButton(
            self.tr("Subtitle Layout"), self, FIF.LAYOUT
        )
        self.layout_button.setFixedHeight(34)
        self.layout_button.setMinimumWidth(125)
        self.layout_menu = RoundMenu(parent=self)
        for layout in ["Translation on top", "Original on top", "Translation only", "Original only"]:
        ...
        self.target_language_button = TransparentDropDownPushButton(
            cfg.target_language.value.value, self, FIF.LANGUAGE
        )
        ...
        self.prompt_button = Action(
            FIF.DOCUMENT, self.tr("Script Prompt"), triggered=self.show_prompt_dialog
        )
        ...
        # 添加开始按钮到水平布局
        self.start_button = PrimaryPushButton(self.tr("Start"), self, icon=FIF.PLAY)
    ...
    def _setup_bottom_layout(self):
        ...
        self.status_label = BodyLabel(self.tr("Please drag subtitle file"), self)
    ...
    def update_info(self, task: SubtitleTask) -> None:
        ...
        self.status_label.setText(self.tr("File loaded"))

    def start_subtitle_optimization(self, need_create_task: bool = True) -> None:
        # 检查是否有任务
        if not self.subtitle_path:
            InfoBar.warning(
                self.tr("Warning"), self.tr("Please load subtitle file first"), duration=3000, parent=self
            )
            return
        ...
        InfoBar.info(
            self.tr("Start Optimization"), self.tr("Start optimizing subtitles"), duration=3000, parent=self
        )
    ...
    def on_subtitle_optimization_finished(
        ...
        # 根据是否进行了翻译显示不同的完成信息
        if self.task and hasattr(self.task, 'subtitle_config') and self.task.subtitle_config.need_translate:
            target_lang = str(self.task.subtitle_config.target_language) if self.task.subtitle_config.target_language else "Chinese"
            InfoBar.success(
                self.tr("Processing Completed"),
                self.tr(f"Subtitle optimization and translation to {target_lang} completed..."),
                duration=3000,
            ...
            InfoBar.success(
                self.tr("Optimization Completed"),
                self.tr("Subtitle optimization completed..."),
                duration=3000,
                position=InfoBarPosition.BOTTOM,
                parent=self.parent(),
            )
    ...
    def on_subtitle_optimization_error(self, error: str) -> None:
        ...
        InfoBar.error(self.tr("Optimization Failed"), self.tr(error), duration=20000, parent=self)
    ...
    def on_file_select(self) -> None:
        ...
        filter_str = f"{self.tr('Subtitle File')} ({subtitle_formats})"

        file_path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Select Subtitle File"), "", filter_str
        )
        if file_path:
    ...
    def on_save_format_clicked(self, format: str) -> None:
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            self.tr("Save Subtitle File"),
            default_name,  # 使用原文件名作为默认名
            f"{self.tr('Subtitle File')} (*.{format})",
        )
        if not file_path:
            return
        ...
        try:
            ...
            InfoBar.success(
                ...
                self.tr("Subtitles saved to: ") + file_path,
                duration=3000,
                parent=self,
            )
            ...
            InfoBar.error(
                self.tr("Save Failed"),
                self.tr("Failed to save subtitle file: ") + str(e),
                duration=5000,
                parent=self,
            )
        ...
        for file_path in files:
            ...
            if is_supported:
                ...
                InfoBar.success(
                    ...
                    self.tr("Import Successful") + os.path.basename(file_path),
                    duration=3000,
                    position=InfoBarPosition.BOTTOM,
                    parent=self,
                )
                ...
                InfoBar.error(
                    ...
                    self.tr("Supported subtitle formats: ") + str(supported_formats),
                    duration=3000,
                    parent=self,
                )
        event.accept()
    ...
    def show_context_menu(self, pos) -> None:
        ...
        # 添加菜单项
        # retranslate_action = Action(FIF.SYNC, self.tr("Retranslate"))
        merge_action = Action(FIF.LINK, self.tr("Merge"))  # Add shortcut tip
    ...
    def merge_selected_rows(self, rows: List[int]) -> None:
        ...
        # 显示成功提示
        InfoBar.success(
            self.tr("Merge Successful"),
            self.tr("Successfully merged selected subtitle rows"),
            duration=1000,
            parent=self,
        )
    ...
    def cancel_optimization(self) -> None:
        ...
        if hasattr(self, "subtitle_optimization_thread"):
            ...
            self.status_label.setText(self.tr("Correction cancelled"))
            InfoBar.warning(
                self.tr("Cancelled"), self.tr("Subtitle correction cancelled"), duration=3000, parent=self
            )
...
class PromptDialog(MessageBoxBase):
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setup_ui()
        self.setWindowTitle(self.tr("Script Prompt"))
        # 连接按钮点击事件
        self.yesButton.clicked.connect(self.save_prompt)

    def setup_ui(self) -> None:
        self.titleLabel = BodyLabel(self.tr("Script Prompt"), self)
