...
class VideoSynthesisInterface(QWidget):
    ...
    def setup_ui(self):
        ...
        self.synthesize_button = PrimaryPushButton(
            self.tr("Start Synthesis"), self, icon=FIF.PLAY
        )
        ...
        self.subtitle_label = BodyLabel(self.tr("Subtitle File"), self)
        self.subtitle_input = LineEdit(self)
        self.subtitle_input.setPlaceholderText(self.tr("Select or drag subtitle file"))
        self.subtitle_input.setAcceptDrops(True)  # 启用拖放
        self.subtitle_button = PushButton(self.tr("Browse"))
        ...
        self.video_label = BodyLabel(self.tr("Video File"), self)
        self.video_input = LineEdit(self)
        self.video_input.setPlaceholderText(self.tr("Select or drag video file"))
        self.video_input.setAcceptDrops(True)  # 启用拖放
        self.video_button = PushButton(self.tr("Browse"))
    ...
    def _setup_command_bar(self):
        ...
        self.soft_subtitle_action.setToolTip(self.tr("Embed soft subtitles in video"))
        ...
        self.need_video_action.setToolTip(self.tr("Whether to generate new video file"))
    ...
    def choose_video_file(self):
        ...
        filter_str = f"{self.tr('Video File')} ({video_formats})"

        file_path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Select Video File"), "", filter_str
        )
        if file_path:
    ...
    def create_task(self):
        ...
        if not subtitle_file or not video_file:
            InfoBar.error(
                self.tr("Error"),
                self.tr("Please select subtitle and video files"),
                duration=3000,
    ...
    def on_video_synthesis_finished(self, task):
        ...
        InfoBar.success(
            self.tr("Success"),
            self.tr("Video synthesis completed"),
            duration=3000,
            position=InfoBarPosition.TOP,
            parent=self,
        )
    ...
    def open_video_folder(self):
        if self.task and self.task.output_path:
            ...
            InfoBar.warning(
                self.tr("Warning"),
                self.tr("No available video folder"),
                duration=2000,
                position=InfoBarPosition.TOP,
                parent=self,
            )
        ...
        for file_path in files:
            ...
            if file_ext in {fmt.value for fmt in SupportedSubtitleFormats}:
                ...
                InfoBar.success(
                    ...
                    self.tr("Subtitle file added to input"),
                    duration=2000,
                    parent=self,
                )
                break
                ...
                InfoBar.success(
                    self.tr("Import Successful"),
                    self.tr("Video file added to input"),
                    duration=2000,
                    parent=self,
                )
                break
                ...
                InfoBar.error(
                    ...
                    self.tr("Please drag video or subtitle file"),
                    duration=3000,
                    parent=self,
                )
